...
public class KafkaDevsStreamProxyTest {
  ...
  void kafkaDevsStreamProxyTest() throws IOException, ExecutionException, InterruptedException {
    ...
    ActorRef<DevsMessage> simulator = testKit.spawn(
    ...
    ActorRef<DevsMessage> generatorReceiver =
        testKit.spawn(KafkaReceiver.create(toSimulatorProbe.getRef(),
            fromSimulatorProbe.getRef(), kafkaConsumerConfig, generatorInputTopic));
