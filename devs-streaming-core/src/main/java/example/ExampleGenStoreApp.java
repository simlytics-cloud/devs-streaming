...
public class ExampleGenStoreApp extends AbstractBehavior<ExampleGenStoreApp.GenStoreApp> {
  ...
  protected Behavior<GenStoreApp> onStart(GenStoreStart start) {
    ...
    ActorRef<DevsMessage> generator =
        new devs.DevsSimulatorProvider<>(new GeneratorModel(0, "generator"))
            .provideSimulator((ActorContext)context, LongSimTime.builder().t(0L).build());

    ActorRef<DevsMessage> storage =
        new devs.DevsSimulatorProvider<>(new StorageModel(new StorageState(StorageStateEnum.S0), "storage"))
            .provideSimulator((ActorContext)context, LongSimTime.builder().t(0L).build());
